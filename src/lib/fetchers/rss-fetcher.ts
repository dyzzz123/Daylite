...
export async function fetchRSS(config: RSSFetcherConfig): Promise<FeedItemInput[]> {
...
export async function fetchMultipleRSS(configs: RSSFetcherConfig[]): Promise<FeedItemInput[]> {
  const results = await Promise.allSettled(configs.map((config) => fetchRSS(config)));

  const allItems: FeedItemInput[] = [];
