...
export async function getFeedItems(params: GetFeedsParams = {}): Promise<FeedItem[]> {
  ...
  // Join with feed_sources to only show items from enabled sources
  let query = `
    SELECT fi.* FROM feed_items fi
    INNER JOIN feed_sources fs ON fi.sourceName = fs.name
    WHERE fs.enabled = 1
  `;
...
export async function getFeedItemsCount(params: GetFeedsParams = {}): Promise<number> {
  ...
  // Join with feed_sources to only count items from enabled sources
  let query = `
  ...
  await db.execute({
    sql: `
      INSERT INTO feed_items (
        id, source, sourceName, title, summary, url, faviconUrl,
