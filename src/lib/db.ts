...
// Migration: Add faviconUrl column if not exists
export async function migrateDatabase() {
  try {
    // Check if faviconUrl column exists in feed_sources
    const sourcesSchema = await db.execute(`
      PRAGMA table_info(feed_sources)
    `);

    const hasFaviconUrlInSources = sourcesSchema.rows.some(
      (row: any) => row.name === 'faviconUrl'
    );

    if (!hasFaviconUrlInSources) {
      console.log('[Migration] Adding faviconUrl column to feed_sources...');
      await db.execute(`
        ALTER TABLE feed_sources ADD COLUMN faviconUrl TEXT
      `);
      console.log('[Migration] ✓ feed_sources updated');
    }

    // Check if faviconUrl column exists in feed_items
    const itemsSchema = await db.execute(`
      PRAGMA table_info(feed_items)
    `);

    const hasFaviconUrlInItems = itemsSchema.rows.some(
      (row: any) => row.name === 'faviconUrl'
    );

    if (!hasFaviconUrlInItems) {
      console.log('[Migration] Adding faviconUrl column to feed_items...');
      await db.execute(`
        ALTER TABLE feed_items ADD COLUMN faviconUrl TEXT
      `);
      console.log('[Migration] ✓ feed_items updated');
    }
