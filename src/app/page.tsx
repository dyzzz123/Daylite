...
export default function DashboardPage() {
  ...
  const [aiEnabled, setAiEnabled] = useState(true); // AI 功能开关
  ...
  // 整合的刷新功能：同时刷新 AI 汇报和信息流
  async function handleRefreshAll() {
    setRefreshLoading(true);
    try {
      // 并行执行：获取新信息流、生成 AI 汇报
      await Promise.all([
        fetch('/api/fetch', { method: 'POST' }), // 抓取新信息
        fetch("/api/summary", {
          method: "POST",
      ...
      // 刷新显示
      await Promise.all([fetchFeeds(), fetchSummary()]);
    } catch (err) {
  ...
  // Handle sources update (refresh feeds and sources)
